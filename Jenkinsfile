pipeline {
  ...
  stages {
    ...
    stage('Commit and Push Changes') {
      steps {
        script {
          ...
          git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@git@github.com:99kairas/go_online_soccer.git
    ...
    stage('Deploy to Remote Server') {
      steps {
        script {
          def targetDir = "D:\PROYEK\Golang\online_soccer\user-service2"
          ...
          ssh -o StrictHostKeyChecking=no -i ${SSH_KEY} ${USERNAME}@${HOST} '
            if [ -d "${targetDir}/.git" ]; then
                ...
                git clone -b "${TARGET_BRANCH}" git@github.com:99kairas/go_online_soccer.git "${targetDir}"
                cd "${targetDir}"
            fi
